...
25.	是否使用过 EXPRESS，KOA，请试写IE下的自定义中间件
...
28.	请写出ES6的decoration, 实现调用函数打印出调用
@logxxx()
...
43.	请解释sting 下的截取STRING 方法，哪些可以使用字典查询
...
57.	请使用JQUERY的 DEFERRED 对象完成PROMISE，请说明哪些版本下有哪些问题。
...
61.	Box sizing 作用，正常文本模式和怪异模式下盒子模型有什么区别
...
其中第一步返回的部分数据在第二个步骤中需要使用到，第二步和第一步的
值需要在第三步中使用

75.	EXPRESS 中实现自动读取一个目录下的ROUTER加载

76.	请用try catch 实现一个LET兼容实现

77.	History replacehistory 接受哪些参数，第二个参数是什么意思

78.	 删除数组内的2-4个内容，并插入新内容S到删除内容处
...
83.	Yarn.lock是怎么生成的，package-lock.json是怎么生成的
...
87.	为什么一个原始类型会有函数，可以操作它

88.	Es6 class 中static干什么用的，相当于原型链中哪些实现
...
98.	请解释下CSS中为什么要加入@CHARSET UTF-8
